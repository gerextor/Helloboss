...
class mainCog(commands.Cog): 
	...
	@commands.command(name='!안녕히')
	async def brother2_(self, ctx):
		if basicSetting[21] != "1":
			return await ctx.send('```보이스를 사용하지 않도록 설정되어 있습니다.```', tts=False)
		return await PlaySound(ctx.voice_client, './sound/안녕히.mp3')
	
	@commands.command(name='!나와라이')
	async def brother2_(self, ctx):
		if basicSetting[21] != "1":
			return await ctx.send('```보이스를 사용하지 않도록 설정되어 있습니다.```', tts=False)
		return await PlaySound(ctx.voice_client, './sound/나와라이.mp3')
